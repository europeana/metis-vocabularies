...
* added properties skos:altLabel and owl:sameAs
-->
...
<xsl:stylesheet version="2.0"
  ...
  <xsl:variable name="targetBio" select=".//gvp:biographyPreferred/@rdf:resource" />
      ...
      <xsl:if test="@rdf:about=$targetId">
        ...
        <edm:Agent>
          ...
          <xsl:if test="not($prefLabelWithLangTag) or not($EnPrefLabel)">
            <xsl:apply-templates
              select="../skos-xl:Label[@rdf:about=$targetLabel]/skos-xl:literalForm" />
          </xsl:if>
