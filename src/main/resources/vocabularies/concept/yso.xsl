...
* converted skos:closeMatch to skos:exactMatch
...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="ysometa:Concept | skos:Concept">
        <skos:Concept>
            ...
            <xsl:for-each select="skos:closeMatch">
                <skos:exactMatch rdf:resource="{ @rdf:resource | */@rdf:about }"/>
            </xsl:for-each>
    ...
    <xsl:template match="*" mode="copy_resource">
        <xsl:element name="{name()}" namespace="{namespace-uri()}">
            <xsl:attribute name="rdf:resource" select="@rdf:resource | */@rdf:about"/>
         </xsl:element>
    </xsl:template>
