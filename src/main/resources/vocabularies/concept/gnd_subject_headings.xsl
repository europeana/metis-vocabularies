...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/rdf:RDF"><!-- Parent mapping: rdf:Description -> skos:Concept -->
        <xsl:for-each select="./rdf:Description[@rdf:about=$targetId]">
            ...
            <xsl:if test="rdf:type/@rdf:resource[.='http://d-nb.info/standards/elementset/gnd#SubjectHeadingSensoStricto']">
                <skos:Concept><!-- Attribute mapping: rdf:about -> rdf:about -->
                    <xsl:if test="@rdf:about">
                        <xsl:attribute name="rdf:about">
                            <xsl:value-of select="@rdf:about"></xsl:value-of>
                        </xsl:attribute>
                    </xsl:if><!-- Tag mapping: gndo:preferredNameForTheSubjectHeading -> skos:prefLabel -->
                    ...
                    </xsl:for-each><!-- Tag mapping: gndo:variantNameForTheSubjectHeading -> skos:altLabel -->
                    ...
                    </xsl:for-each><!-- Tag mapping: gndo:gndSubjectCategory -> skos:exactMatch -->
                    <xsl:for-each select="./gndo:gndSubjectCategory">
                        <skos:exactMatch><!-- Attribute mapping: rdf:resource -> rdf:resource -->
